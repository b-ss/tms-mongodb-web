...
class SyncToPool extends Base {
  ...
  async syncOrder() {
    ...
    if (
      ...
      return new ResultFault('文件没有指定集合列定义')
    // 校验必须列
    ...
    // 分批次插入, 一次默认插入200条
    let orders = await colle
      .find(find)
      .limit(parseInt(execNum))
      .toArray()

    let rst = await this.fnSync(orders, dbSchema, colle, {
      db: existDb.name,
      cl,
      operate_type
    })
    ...
    let rst = orders.map(async order => {
      ...
      if (order.pro_type === '1') {
        ...
        if (order.costtype_yly === '1') {
          data.money_a = order.dismoney_a_yly
            ? order.dismoney_a_yly
            : order.money_a_yly
          data.money_b = order.dismoney_b_yly
            ? order.dismoney_b_yly
            : order.money_b_yly
          data.money_c = order.dismoney_c_yly
            ? order.dismoney_c_yly
            : order.money_c_yly
          data.money_d = order.dismoney_d_yly
            ? order.dismoney_d_yly
            : order.money_d_yly
          data.money_e = order.dismoney_e_yly
            ? order.dismoney_e_yly
            : order.money_e_yly
          data.money_f = order.dismoney_f_yly
            ? order.dismoney_f_yly
            : order.money_f_yly
          data.money_ex = order.disovermoney_yly
            ? order.disovermoney_yly
            : order.overmoney_yly
      ...
      if (order.pro_type === '2') {
        ...
        let data = {
          ...
          cost_month: order.discostmonth_yzj
            ? order.discostmonth_yzj
            : order.costmonth_yzj,
          cost_call: order.discostcall_yzj
            ? order.discostcall_yzj
            : order.costcall_yzj,
        ...
        if (order.recyzj_flag === 'Y') {
          ...
          data.extern_flag = order.extern_flag_yzj
          if (order.extern_flag_yzj === '1') {
            data.requestUrl = order.call_url_yzj
          }
        }
        Object.assign(postData, data)
      }
      ...
      if (order.pro_type === '3') {
        ...
        let data = {
          cost_month: order.discost_month_gzh
            ? order.discost_month_gzh
            : order.cost_month_gzh,
          cost_call: order.discost_call_gzh
            ? order.discost_call_gzh
            : order.cost_call_gzh,
        ...
        if (order.biz_function && order.biz_function.indexOf('1') !== -1) {
          ...
          if (order.costtype_gzh === '1') {
            data.money_a = order.dismoney_a_gzh
              ? order.dismoney_a_gzh
              : order.money_a_gzh
            data.money_b = order.dismoney_b_gzh
              ? order.dismoney_b_gzh
              : order.money_b_gzh
            data.money_c = order.dismoney_c_gzh
              ? order.dismoney_c_gzh
              : order.money_c_gzh
            data.money_d = order.dismoney_d_gzh
              ? order.dismoney_d_gzh
              : order.money_d_gzh
            data.money_e = order.dismoney_e_gzh
              ? order.dismoney_e_gzh
              : order.money_e_gzh
            data.money_f = order.dismoney_f_gzh
              ? order.dismoney_f_gzh
              : order.money_f_gzh
            data.money_ex = order.disovermoney_gzh
              ? order.disovermoney_gzh
              : order.overmoney_gzh
          } else {
            data.duration_price = order.dismoney_time_gzh
              ? order.dismoney_time_gzh
              : order.money_time_gzh
          }
        }
        if (order.biz_function && order.biz_function.indexOf('2') !== -1) {
          data.cost_msg = order.discost_msg_gzh
            ? order.discost_msg_gzh
            : order.cost_msg_gzh
      ...
      return new Promise(async resolve => {
        ...
        if (rstSync.status === true) {
          ...
          await colle.updateOne(
            { _id: ObjectId(order._id) },
            { $set: { pool_sync_time: current, pool_sync_status: returnMsg } }
          )
          ...
          await colle.updateOne(
            { _id: ObjectId(order._id) },
            { $set: { pool_sync_time: syncTime, pool_sync_status: returnMsg } }
          )
